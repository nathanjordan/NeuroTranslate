...
//if (TIN->Node == 0) printf ("\nparse/ConvertIn: starting conversion\n");
  ...
  AR->Brain->Events   = ConvertList (brain->EventNames, brain->nEvents,  SymTree, "Event");
...
void ConvertEvent( TREENODE *SymTree, ARRAYS *AR )
{
    int eventIndex;
    T_EVENT *activeEvent;
    AR->nEvents = TIN->nEvent;
    
    if( AR->nEvents > 0 )
    {
        AR->Events = (T_EVENT **) calloc ( AR->nEvents, sizeof(T_EVENT*) );
        activeEvent = TIN->Event;
        
        for( eventIndex=0; eventIndex < AR->nEvents; eventIndex++ )
        {
            AR->Events [ activeEvent->L.idx ] = activeEvent;
            
            //lookup column/layer/cell/cmp indices
            activeEvent->cellGroup[0] = getName (&(activeEvent->L), activeEvent->cellGroupNames[0], SymTree, "Column");
            activeEvent->cellGroup[1] = getName (&(activeEvent->L), activeEvent->cellGroupNames[1], SymTree, "Layer");
            activeEvent->cellGroup[2] = getName (&(activeEvent->L), activeEvent->cellGroupNames[2], SymTree, "Cell");
            activeEvent->cellGroup[3] = getCmpIdx ( activeEvent->cellGroupNames[2], activeEvent->cellGroupNames[3], SymTree, &(activeEvent->L) );

            //lookup synapse definition
            activeEvent->synapse = getName (&(activeEvent->L), activeEvent->synapseName, SymTree, "Synapse");
            
            //make sure the file referenced exists
            FILE *fin = fopen( activeEvent->file, "r" );
            if( !fin )
            {
                fprintf( stderr, "Error: unable to open synapse USE override file %s\n", activeEvent->file );
                TIN->nDataErr++;
            }
            else
                fclose(fin);
            
            //if more than just an override event come into existence, probably do something with it now
            
            //advance to next event
            activeEvent = activeEvent->L.next;
        }
    }
}
