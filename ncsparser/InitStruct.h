...
struct T_BRAIN
{
  ...
  T_RECURRENT *recurrentList;
          ///Number of event objects used in the brain
  int nEvents;
          ///Linked List of event object names which should be used
  LIST *EventNames;
          ///array of indexes to event objects
  int *Events;
};
...
struct T_SYNAPSE
{
  ...
  double InitRSE[2];
          ...
          ///If this synapse will be toggling Hebbian during the simulation, this holds the number of times it will happen
  int nToggles;
          ///For synapse hebbian toggling, array of simulation times (seconds) when the toggles are applied
  double *toggleTimes;
          ///For synapse hebbian toggling, array of toggle modes to apply when toggling time comes 'round ( 0 - NONE; 1 - use declared learning )
  int *toggleModes;
 
    // Added 8/12/10 Joshua Hegie 
          ///If this synapse will be toggling SFD during the simulation, this holds the number of times it will happen
  int nSfdToggles;
          ///For synapse SFD toggling, array of simulation times (seconds) when the toggles are applied
  double *sfdToggleTimes;
          ///For synapse SFD toggling, array of toggle modes to apply when toggling time comes 'round ( 0 - NONE; 1 - use declared learning )
  int *sfdToggleModes;
};
...
struct T_SYNLEARN
{
  ...
  int Learning;
          ///The type of learning shape handled by this object - TRIANGLE, EXPONENT
  int Learning_Shape;
          ///Random Number generator seed
  long Seed;
...
struct T_STIMULUS
{
  ...
  double Rate;              // for NOISE
  double Tau;               // for NOISE
  double Correl;            // for NOISE
...
struct T_REPORT
{
  ...
  int ReportOn;
          /// Represents the cell sequence saved in report data written to a report file.
          /// Sample values are either ORIGINAL (first to last), which is default, or RANDOM.
          /// Currently optional to maintian backwards compatibility.
  int CellSequence;
...
/**
 * Structure to hold parsed events which are to occur at some point during the simulation
 * For now, only a USE over ride for synapses is defined.  Maybe the Synapse Hebbian toggle
 * should also be moved here?
 */
struct T_EVENT
{
        ///Structure to organize Events into a linked list and manage fields common to all objects
    LOCATOR L;
        ///Name of the Synapse which will be affected by the event
    char *synapseName;
        ///Index of Synapse object in final array
    int synapse;
        ///Names of the column, layer, cell, and cmp which will own the synapse
    char *cellGroupNames[4];
        ///Indexes of the column, layer, cell, and cmp
    int cellGroup[4];
        ///The action to perform (currently only OVERRIDE)
    int action;
        ///File where value may be retrieved from
    char *file;
        ///Time when the action should occur (should this become a list to allow for multiple times?)
    double time;
};
