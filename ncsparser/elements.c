...
T_BRAIN *makebrain ()
{
  ...
  p->nColumns    = p->nStInject  = p->nReports    = p->nConnect = p->nEvents = 0;
  p->ColumnNames = p->StInjNames = p->ReportNames = p->EventNames = NULL;
  p->Columns     = NULL;
  p->StInject    = NULL;
  p->Reports     = NULL;
  p->CnList      = NULL;
...
T_SYNAPSE *makesynapse ()
{
  ...
  p->sfdToggleTimes = NULL;
  p->sfdToggleModes = NULL;
  p->nSfdToggles = 0;
...
T_REPORT *makereport ()
{
  ...
  p->ReportOn = INVALID;
  p->CellSequence = ORIGINAL; // Making ORIGINAL as default for backwards compatibility
  p->Seed = 0;
  p->reportFlag = 0;
...
T_EVENT *makeevent()
{
    ...
    p = (T_EVENT*) allocElem ((void*) TIN->Event, sizeof (T_EVENT), TK_REPORT );
    ...
    p->synapse = 0;
    p->synapseName = NULL;
    p->cellGroupNames[0] = p->cellGroupNames[1] = p->cellGroupNames[2] = p->cellGroupNames[3] = NULL;
    p->cellGroup[0] = p->cellGroup[1] = p->cellGroup[2] = p->cellGroup[3] = 0;
    p->file = NULL;
    p->time = 0.0;
    
    //If NCS comes to support multiple actions, this would probably be a constant used to distiguish them
    p->action = 0;
