...
public NCSSCanner(java.io.Reader r, parser yyparser) {
	this(r);
...
\-?{DIGIT}+            { yylval.ival = Integer.parseInt(yytext); return (INTEGER); }

\-?{DIGIT}+"."{DIGIT}*({E})? { yylval.rval = new Float(yytext); return (REAL); }    /* catches 1. */
\-?{DIGIT}*"."{DIGIT}+({E})? { yylval.rval = new Float(yytext); return (REAL); }    /* catches .01 */
\-?{DIGIT}+{E}         { yylval.rval = new Float(yytext); return (REAL); }
...
Km           { yylval.sval = yytext; return (TK_Km); }
Kahp         { yylval.sval = yytext; return (TK_Kahp); }
Ka           { yylval.sval = yytext; return (TK_Ka); }
Na           { yylval.sval = yytext; return (TK_Na); }
Knicd        { yylval.sval = yytext; return (TK_Knicd); }
