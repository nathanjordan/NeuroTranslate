...
/*----------------------------------------------------------------------------*/
int LearnShape( char *str )
{
  int rc;

  rc = LEARN_TRI;
  if (strcasecmp (str, "TRIANGLE") == 0)
  rc = LEARN_TRI;
  else if (strcasecmp (str, "EXPONENT") == 0)
  rc = LEARN_EXP;
...
int PatternCode (char *str)
{
  ...
  if (strcasecmp (str, "LINEAR") == 0)
    ...
    rc = INPUTSTIM;
  else if (strcasecmp (str, "FILE_BASED_DIRECT") == 0)
    rc = FILEBASEDDIRECT;
  else 
  {
...
/*----------------------------------------------------------------------------*/
int CellSequenceCode (char *str)
{
  int rc; // Cell Sequence Code
  ...
  {
    printerr ("%s:%d: Invalid value for keyword CELL_SEQUENCE '%s'\n", TIN->file, TIN->line, str);
    rc = INVALID;
    TIN->nParseErr++;
  }
